...
Protected targets should declare `authScope` and `authenticatedSelector` in `scripts/app-screenshots/targets.ts`.
Manual auth states are bearer-secret local files under `.local/app-screenshot-auth/`.
